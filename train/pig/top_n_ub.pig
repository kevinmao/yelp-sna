...
-- random
--
A = FOREACH PredictedGrouped GENERATE COUNT(Predicted) as num_rows;
random = SAMPLE Predicted 0.0001+$TOPN/num_rows;
...
TruePositive = FOREACH J GENERATE
    ...
    top_random::sim_random AS sim_random,
    ((TestCore::user_id is null) ? 0 : 1) AS tp_flag;
TruePositive = ORDER TruePositive BY sim_random DESC, user_id, business_id;
STORE TruePositive into '$TruePositive.random' using PigStorage('\t', '-schema');
