...
def ub_core_train(inputFile):
    ...
    Core_User = set()
    for u in User:
        if UserDegree[u] < MIN_DEGREE_USER: continue
        Core_User.add(u)
    # core business
    Core_Business = set()
    for b in Business:
        if BusinessDegree[b] < MIN_DEGREE_BUSINESS: continue
        Core_Business.add(b)
...
def main(args):
    ...
    review_train_maxwcc_file = args.review_train_maxwcc
    review_train_core_file = args.review_train_core
    review_test_core_file = args.review_test_core
    
    Core_User, Core_Business, Core_Edge = ub_core_train(review_train_maxwcc_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--review_train_maxwcc', metavar='FILE', required = True, help='test data')
